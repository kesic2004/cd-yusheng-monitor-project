...
public interface DeviceMapper extends BaseMapper<Device> {
   ...
   Device findDeviceByClientIdAndAddress(@Param("clientid") Integer clientid, @Param("addresscode") Integer addresscode, @Param("province") Integer province, @Param("city") Integer city, @Param("station") Integer station);
    ...
    @Select("select id from device where sid = #{sid} and machineno = #{machineno} and addresscode = #{addresscode} order by id desc limit 1")
    Integer getIdByCode(@Param("sid") Integer stationId, @Param("machineno") int machineNo, @Param("addresscode") int address);
}
